 ...
 * Make sure "Wiki loaded" links reflect option panel changes? (extra "params" object)
 *
 * ## TERTIARY ##
 ...
 *
 * ## NOT REALLY FEASIBLE (at this time) ##
 * Show if a RC has been patrolled -- Not possible since it need to know you are logged in, and if no patrol right is found, whole request is invalid.
 */
...
//######################################
	...
	var FAVICON_BASE = "http://www.google.com/s2/favicons?domain="; // Fallback option (encase all other options are unavailable)
	var AUTO_REFRESH_LOCAL_STORAGE_ID = "RecentChangesMultiple-autorefresh";
	var LOADER_IMG = "http://slot1.images.wikia.nocookie.net/__cb1421922474/common/skins/common/images/ajax.gif";
	/*
	 ...
	 * 			 POTENTIAL ISSUES:
	 * 			 	* Script cannot check proper use of "{{GENDER}}", so just does male.
	 */
	var i18n = {
		TEXT: {
			en: { // English (ENGLISH)
...
//######################################
	...
	module.start = function() {
		...
		Utils.forEach(tWrappers, function tRCM_start_createRCMs(pNode, pI, pArray){
			module.rcmList.push( new RecentChangesMultiple(pArray[pI], pI).init() );
		});
...
//######################################
// #### RecentChangesMultiple - Module core ####
...
//######################################
	var RecentChangesMultiple = (function()
	{
		// Static Constants
		RecentChangesMultiple.LOADING_ERROR_RETRY_NUM_INC = 5;
		
		// Constructor
		function RecentChangesMultiple(pWrapper, pModID) {
			/***************************
			 * HTML Elements/Nodes
			 ***************************/
			...
			this.hideusers				= null; // {array} List of users to hide across whole RecentChangesMultiple
			this.onlyshowusers			= null; // {array} Only show these users' edits across whole RecentChangesMultiple
			this.chosenWikis			= null; // {array} Wikis for the script to load
		...
		RecentChangesMultiple.prototype.dispose = function() {
		...
		RecentChangesMultiple.prototype.init = function() {
			/***************************
			 * Data provided to script
			 ***************************/
			...
			// List of users to hide across whole RecentChangesMultiple
			...
			// Only show these users' edits across whole RecentChangesMultiple
		...
		RecentChangesMultiple.prototype._start = function() {
			var self = this;
			...
			this.ajaxID++;
			this.loadingErrorRetryNum = RecentChangesMultiple.LOADING_ERROR_RETRY_NUM_INC;
			...
			this.statusNode.innerHTML = "<img src='"+LOADER_IMG+"' /> "+i18n.TEXT.loading+" (<span class='rcm-load-perc'>0%</span>)";
		};
		
		RecentChangesMultiple.prototype.refresh = function() {
			...
			RecentChangesMultiple.closeDiff();
			
			this._start();
		};
		
		// Separate method so that it can be reused if the loading failed
		RecentChangesMultiple.prototype.loadWiki = function(pWikiInfo, pTries, pID, pDelayNum) {
			var self = this;
		...
		RecentChangesMultiple.prototype.onWikiLoaded = function(pData, pWikiInfo, pTries, pID, pFailStatus) {
			var self = this;
			...
			if(!!pData && pData.error && pData.query == null) {
				...
				Utils.newElement("button", { innerHTML:Utils.formatString(i18n.TEXT.tryMoreTimes, 1) }, self.statusNode).addEventListener("click",
					function tHandler(pData){
						...
						self.erroredWikis = [];
						self.statusNode.innerHTML = "<img src='"+LOADER_IMG+"' /> "+i18n.TEXT.loading+" (<span class='rcm-load-perc'>"+self.calcLoadPercent()+"%</span>)";
					}
				);
				...
				if(pTries < this.loadingErrorRetryNum) {
					...
					if(this.erroredWikis.length === 0) {
						...
						Utils.newElement("button", { innerHTML:Utils.formatString(i18n.TEXT.tryMoreTimes, RecentChangesMultiple.LOADING_ERROR_RETRY_NUM_INC) }, self.statusNode).addEventListener("click",
							function tHandler(pData){
								self.loadingErrorRetryNum += RecentChangesMultiple.LOADING_ERROR_RETRY_NUM_INC;
								pData.target.removeEventListener("click", tHandler);
		...
		RecentChangesMultiple.prototype.parseWiki = function(pData, pLogData, pWikiInfo, pTries) {
			var self = this;
		...
		RecentChangesMultiple.prototype.onWikiParsingFinished = function(pWikiInfo) {
		...
		RecentChangesMultiple.prototype.rcmChunkStart = function() {
		...
		RecentChangesMultiple.prototype.rcmChunk = function(pIndex, pLastDay, pLastMonth, pContainer, pID) {
		...
		RecentChangesMultiple.prototype.finishScript = function() {
		...
		RecentChangesMultiple.prototype.loadExtraInfo = function(pID) {
			if(pID != this.ajaxID) { return; }
			if(this.secondaryWikiData.length == 0) { if(module.debug){ console.log("[RecentChangesMultiple](loadExtraInfo) All loading finished."); } return; }
			var self = this;
		...
		//######################################
		RecentChangesMultiple.prototype.addRefreshButtonTo = function(pParent) {
			var self = this;
		...
		RecentChangesMultiple.prototype.addAutoRefreshInputTo = function(pParent) {
			var self = this;
		...
		RecentChangesMultiple.prototype.calcLoadPercent = function() {
		...
		RecentChangesMultiple.prototype.parseRCParams = function(pData, pExplodeOn, pSplitOn) {
			var tRcParams = {};
		...
		RecentChangesMultiple.prototype.getDefaultRCParams = function() {
			return {
		...
		RecentChangesMultiple.previewDiff = function(pPageName, pageID, pAjaxUrl, pDiffLink, pUndoLink) {
			...
			if ($('#DiffView').length == 0) {
				...
				tButtons.push({
					...
					handler: RecentChangesMultiple.closeDiff
				});
			...
			$('#DiffView').html("<div style='text-align:center; padding:10px;'><img src='"+LOADER_IMG+"'></div>");
		}
		
		RecentChangesMultiple.closeDiff = function() {
			if($('#DiffView').length != 0) {
				$('#page-viewer').closeModal();
			}
		}
		
		return RecentChangesMultiple;
	})();
//}END RecentChangesMultiple - Module core
...
//######################################
	var WikiData = (function()
	{
		...
		// Constructor
		function WikiData(pRCM) {
			this.rcm = pRCM; // Keep track of what RecentChangesMultiple this data is attached to.
			...
			this.rcParamsBase		= null; // {object} Works the same as this.rcm.rcParams but for only this wiki.
			this.rcParams			= null; // {object} Combination of this.rcParamsOriginal and this.rcm.rcParams to get final result.
		...
		WikiData.prototype.initListData = function(pNode) {
			...
			if(this.servername.indexOf("/") > -1) {
				this.rcm.resultCont.innerHTML = "<div style='color:red; padding:4px 5px; background:rgba(0,0,0,0.1);'>"+ Utils.formatString(i18n.TEXT.incorrectFormatLink, this.servername)+"</div>";
			...
			for(var i = 1; i < tWikiDataRaw.length; i++) {
				...
				if(tWikiDataSplit.length > 1) {
					if(tWikiDataSplit[0] == "params") {
						this.rcParamsBase = this.rcm.parseRCParams(tWikiDataSplit[1], ",", ":");
					}
		...
		WikiData.prototype.initAfterLoad = function(pQuery) {
			 ...
			 ***************************/
			if(this.favicon == null) {
				this.favicon = FAVICON_BASE+this.servername;
			}
		}
		...
		WikiData.prototype.setupRcParams = function() {
			...
			if(this.rcParamsBase != null) {
				this.rcParams = $.extend( this.rcm.rcParamsBase, this.rcParamsBase );
				...
				this.rcParams = $.extend( this.rcm.getDefaultRCParams(), this.rcParams );
			} else {
				this.rcParams = this.rcm.rcParams;
			}
			self = null;
		}
		...
		WikiData.prototype.getApiUrl = function() {
			...
			} else if(this.rcm.hideusers.length > 0) {
				tUser = this.rcm.hideusers[0];
			 ...
			 * Get the site info (Once per RCM)
			 ***************************/
			if(this.needsSiteinfoData) {
				 ...
				 * Get favicon url for wiki (needed for wikis below V1.23 [Added to siteinfo]) (Once per RCM)
				 ***************************/
			 ...
			 * If user logged in, get info for this wiki (Once per RCM)
			 ***************************/
...
//######################################
	var RCMOptions = (function(){
		...
		RCMOptions.prototype.init = function(pElem) {
			...
			if(mw.config.get("wgUserName") && this.rcm.hideusers.indexOf(mw.config.get("wgUserName")) != -1) {
		...
		RCMOptions.prototype.refresh = function() {
			...
			var tLimit = this.rcm.rcParams.limit;
			...
			var tDays = this.rcm.rcParams.days;
			 ...
			 * Checkboxes
			 ***************************/
			this.minorEditsCheckbox.checked = !this.rcm.rcParams.hideminor;
			this.botsCheckbox.checked = !this.rcm.rcParams.hidebots;
			this.anonsCheckbox.checked = !this.rcm.rcParams.hideanons;
			this.usersCheckbox.checked = !this.rcm.rcParams.hideliu;
			this.myEditsCheckbox.checked = !this.rcm.rcParams.hidemyself;
			this.groupedChangesCheckbox.checked = !this.rcm.rcParams.hideenhanced;
			this.logsCheckbox.checked = !this.rcm.rcParams.hidelogs;
		}
		...
		RCMOptions.prototype._onChange_hideanons = function(pEvent) {
			...
			if(pEvent.target.checked == false && this.usersCheckbox.checked == false) {
				this.rcm.rcParams["hideliu"] = false;
		...
		RCMOptions.prototype._onChange_hideliu = function(pEvent) {
			...
			if(pEvent.target.checked == false && this.anonsCheckbox.checked == false) {
				this.rcm.rcParams["hideanons"] = false;
...
//######################################
	var RCData = (function()
	{
		...
		// Constructor
		function RCData(pWikiInfo, pRCM) {
			this.rcm = pRCM; // Keep track of what RecentChangesMultiple this data is attached to.
			this.wikiInfo = pWikiInfo; // Keep track of what Wiki this data belongs to.
		...
		RCData.prototype.time = function() {
			return Utils.pad(Utils.getHours(this.date, this.rcm.timezone),2)+":"+Utils.pad(Utils.getMinutes(this.date, this.rcm.timezone),2);
		};
		...
		RCData.prototype.getLogTimeStamp = function(pDate) {
			return ""
				+ 			Utils.pad( Utils.getHours(pDate, this.rcm.timezone), 2 )
				+ ":" +		Utils.pad( Utils.getMinutes(pDate, this.rcm.timezone), 2 )
				+ ", " +	Utils.pad( Utils.getDate(pDate, this.rcm.timezone), 2 )
				+ " " +		mw.config.get('wgMonthNames')[Utils.getMonth(pDate, this.rcm.timezone)+1]
				+ " " +		Utils.getYear(pDate, this.rcm.timezone)
			;
		}
		
		return RCData;
	})();
//}END Recent Change Data
	...
	var RCList = (function()
	{
		...
		RCList.prototype.shouldGroupWith = function(pRC) {
			if(this.wikiInfo.servername == pRC.wikiInfo.servername
				&& this.type == pRC.type
				&& Utils.getMonth(this.date, this.rcm.timezone) == Utils.getMonth(pRC.date, pRC.rcm.timezone)
				&& Utils.getDate(this.date, this.rcm.timezone) == Utils.getDate(pRC.date, pRC.rcm.timezone)
			) {
				switch(this.type) {
		...
		RCList.prototype.toHTML = function(pIndex) {
			if(this.rcm.rcParams.hideenhanced) {
